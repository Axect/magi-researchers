# {title}

**Date**: {date} | **Domain**: {domain} | **Author**: Claude Research Workflow

---

## 1. Research Background

{background}

## 2. Brainstorming Summary

### Key Ideas
{brainstorm_summary}

### Cross-Check Highlights
{crosscheck_highlights}

## 3. Methodology

{methodology}

## 4. Implementation

### Architecture
{architecture}

### Key Components
{components}

## 5. Results & Visualization

{results}

## 6. Testing

### Test Strategy
{test_strategy}

### Test Results
{test_results}

## 7. Conclusion

{conclusion}

---

## Appendix

### A. Full Brainstorming Records
- [Gemini Ideas](brainstorm/gemini_ideas.md)
- [Codex Ideas](brainstorm/codex_ideas.md)
- [Cross-Check: Gemini on Codex](brainstorm/gemini_review_of_codex.md)
- [Cross-Check: Codex on Gemini](brainstorm/codex_review_of_gemini.md)
- [Synthesis](brainstorm/synthesis.md)

### B. Research Plan
- [Research Plan](plan/research_plan.md)

### C. Source Code
- [Source Directory](src/)

### D. Test Code
- [Test Directory](tests/)

### E. MAGI Process Artifacts
- [Scoring Weights](brainstorm/weights.json)
- [Expert Personas](brainstorm/personas.md)
- [Gemini Debate Round 2](brainstorm/debate_round2_gemini.md) *(if `--depth high`)*
- [Codex Debate Round 2](brainstorm/debate_round2_codex.md) *(if `--depth high`)*

### F. Quality Assurance
- [Murder Board](plan/murder_board.md)
- [Mitigations](plan/mitigations.md)
- [Plan Phase Gate](plan/phase_gate.md)
- [Implementation Phase Gate](src/phase_gate.md) *(if available)*
- [Test Phase Gate](tests/phase_gate.md) *(if available)*
